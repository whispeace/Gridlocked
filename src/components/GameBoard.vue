<template>
  <div class="game-container">
    ...
    <div class="main-section">
      ...
      <PlayerPanel 
        :player-id="'player1'" 
        :is-active="currentPlayerId === 'player1'"
        ...
        <div class="grid">
          <div v-for="y in 3" :key="`row-${y}`" class="grid-row">
            ...
            <div 
              ...
              :key="`cell-p1-${x - 1}-${y - 1}`" 
              class="grid-cell"
              :class="getCellClasses(x - 1, y - 1)"
              @click="handleCellClick(x - 1, y - 1)"
              @mouseover="handleCellHover(x - 1, y - 1)"
              @mouseleave="handleCellLeave"
            >
              <div v-if="isPlayerAt('player1', x - 1, y - 1)" class="player-marker p1">
                <span>P1</span>
                <div v-if="gameEngine.gameState.value.players.player1?.isDefending" class="shield-indicator"></div>
              </div>
              <div v-if="isPlayerAt('player2', x - 1, y - 1)" class="player-marker p2">
                <span>P2</span>
                <div v-if="gameEngine.gameState.value.players.player2?.isDefending" class="shield-indicator"></div>
              </div>

              <div v-if="getCellTerrain(x - 1, y - 1).type !== 'empty'" class="terrain-marker" :class="getCellTerrain(x - 1, y - 1).type">
            ...
            <div 
              ...
              :key="`cell-p2-${x + 2}-${y - 1}`" 
              class="grid-cell"
              :class="getCellClasses(x + 2, y - 1)"
              @click="handleCellClick(x + 2, y - 1)"
              @mouseover="handleCellHover(x + 2, y - 1)"
              @mouseleave="handleCellLeave"
            >
              <div v-if="isPlayerAt('player1', x + 2, y - 1)" class="player-marker p1">
                <span>P1</span>
                <div v-if="gameEngine.gameState.value.players.player1?.isDefending" class="shield-indicator"></div>
              </div>
              <div v-if="isPlayerAt('player2', x + 2, y - 1)" class="player-marker p2">
                <span>P2</span>
                <div v-if="gameEngine.gameState.value.players.player2?.isDefending" class="shield-indicator"></div>
              </div>

              <div v-if="getCellTerrain(x + 2, y - 1).type !== 'empty'" class="terrain-marker" :class="getCellTerrain(x + 2, y - 1).type">
      ...
      <PlayerPanel 
        :player-id="'player2'" 
        :is-active="currentPlayerId === 'player2'"
...
const handleCellClick = (x: number, y: number) => {
  ...
  if (selectedAction.value === 'move' && isMoveTarget(x, y)) {
    ...
    // Если игрок кликнул на своей фигуре без выбранного действия, предлагаем движение
    selectAction('move');
    actionTooltip.value = 'Выберите, куда переместиться';
    setTimeout(() => { actionTooltip.value = ''; }, 3000);
  } else if (!selectedAction.value) {
    // Если игрок кликнул на случайной клетке без выбранного действия, даем подсказку
